...
def _run_training(harness: SafetyHarness, ip: str, port: int, args) -> int:
    ...
    remote_cmd = (
        "set -uo pipefail && "
        "cd ~/ors && "
        "python3 -m venv venv-cloud --system-site-packages 2>&1 | tail -3 && "
        ...
        "echo '--- torch check (system site-packages) ---' && "
        "python3 -c 'import torch; print(\"torch\", torch.__version__, \"cuda\", torch.cuda.is_available())' || "
        "(echo '--- torch missing, installing 2.5 ---' && "
        " pip install 'torch==2.5.*' 'torchvision<0.21' --index-url https://download.pytorch.org/whl/cu121 "
        " 2>&1 | tee /tmp/ors-torch-install.log | tail -5) && "
