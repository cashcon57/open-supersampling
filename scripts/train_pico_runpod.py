"""Submit an OSS-Pico training job to RunPod H100 with NoiseBase data from R2.

Usage:
    python scripts/train_pico_runpod.py [--runpod-key KEY] [--r2-bucket NAME]
...
def _build_start_cmd(r2_bucket: str, epochs: int, batch_size: int) -> str:
    return (
        ...
        "mkdir -p /out/pico_run && "
        f"python -m oss.train.train_pico "
        f"--data-root /data/noisebase "
        f"--out /out/pico_run "
        f"--epochs {epochs} "
        f"--batch-size {batch_size} && "
        f"rclone copy /out/pico_run/ r2:{r2_bucket}/checkpoints/pico/ "
        "--config /root/.config/rclone/rclone.conf "
