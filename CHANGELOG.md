...
- 3 CUDA-specific renderer tests fail on the 3080 Ti due to gsplat 1.4.0 API drift from the Image-GS pin. Reference backend works on CUDA, so this is bounded scope; targeted debug task post-Sprint 1 close-out. Patched the inverse-scale convention in `rasterizer.py` (commit `d76ee60`); deeper drift remains.
