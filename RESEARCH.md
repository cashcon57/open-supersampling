...
**⚠ Research / teacher vs end-user inference model.** The dashboard runs (shown on [opensupersampling.com](https://opensupersampling.com)) — v5-pixel-temporal, v6.1-pico-001, v6.2-pico-002 (stopped early at step 74K on 2026-05-12 due to GPU reclaim, preserved as the v7 baseline), and the currently-training **v7.0-pico-005** (N-D Gaussian canvas with native time axis + parent-child loss-adaptive density + OSS-FX frame extrapolation) — are **research / teacher** models built on the HAT-Tiny / HAT-Small / HAT-L backbones. HAT-Tiny FP16 eager forward measured on RTX 3080 Ti (idle) is **54.5 ms at 270×480 LR** and **1,890 ms at 1920×1080 LR**, orders of magnitude above any real-time game-upscaling budget (DLSS 4 FP8 and FSR 4 sit at <2 ms). The end-user inference model in every tier described below is a **distilled student** (nano-CNN, ≤0.4M–2M params depending on tier) trained against these teacher checkpoints and exported to TensorRT FP8 with custom cross-vendor kernels. That student has not yet been trained. See [H006](docs/research/hypotheses/H006-hat-tiny-actual-ms.md) for the kill-HAT-from-inference measurement, and the [Phase 4 council 1–4 ms budget reassessment](docs/research/2026-05-08-phase4-msframe-council/03-1-4ms-budget-reassessment.md) for the unanimous council decision.
