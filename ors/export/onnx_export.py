...
def export(ckpt_path: Path, out_path: Path, validate: bool = True):
    ...
    torch.onnx.export(
        model,
        (color_lr, depth_lr, motion_lr, normals_lr, history_hr, hidden_zero),
        str(out_path),
        opset_version=17,
        input_names=["color_lr", "depth_lr", "motion_lr", "normals_lr", "history_hr", "hidden_state"],
    ...
    if validate:
        _validate_parity(model, out_path, color_lr, depth_lr, motion_lr, normals_lr, history_hr, hidden_zero)


def _validate_parity(model, onnx_path, color_lr, depth_lr, motion_lr, normals_lr, history_hr, hidden_zero):
    ...
    # PyTorch reference
    with torch.no_grad():
        rgb_pt, hidden_pt = model(color_lr, depth_lr, motion_lr, normals_lr, history_hr, hidden_zero)
    ...
    # Tolerances suitable for FP32-traced graph; tighten if you re-export at FP16
    ...
    assert rgb_diff < 1e-4, f"RGB parity failed: {rgb_diff}"
