...
| **v6.1 architecture (active)** | **Training now — Pico tier, started 2026-05-07** | Adds randomized per-frame spawner tile offsets + overlapping rasterizer tiles with cosine feathering; both eliminate the v6 grid period structurally. v6.1-Pico-001 running on 3080 Ti since 2026-05-07 12:00 with dense cold-start checkpoints (step 100 / 500 / 1K / 1.5K / … / 5K, then every 5K). Background: [v6.1 memo](docs/superpowers/experiments/2026-05-07-v6.1-pico-grid-artifact-architectural-fix.md). **Latency targets in this README are conditional on v6.2 architectural changes — see next row.** |
| **v6.2 architecture (committed 2026-05-08)** | **Spec written, implementation queued** | v6.1 in training cannot hit DLSS+FG-class 3-5ms total without further changes. v6.2 cuts HAT-Tiny from inference (replaces with ≤1M distilled student in TensorRT INT8/FP8), replaces F=64 raster with R=4-8 latent splat, deletes global cross-attention from the hot path (concat-fusion + 1×1 conv replaces it; local top-K=16 only on disocclusion tiles), adopts conic row-recurrence in the inner loop, custom counting-sort tile bin (no `torch.sort`), and CUDA Graph capture of the whole frame DAG. Reproject-first: Gaussians are an L2-friendly residual cache layered on TAA-style reprojection, not the primary image generator. **Hardware tiers**: Halo (5090/4090) → Mid-high (3080 Ti / 4070 mobile, the reference) → Mainstream → Entry ML → No-ML (shader-only variant ships alongside v6.2). Spec: [`docs/architecture/2026-05-08-v62-arch-v4-spec.md`](docs/architecture/2026-05-08-v62-arch-v4-spec.md). Skeptical priority stack: [`docs/research/2026-05-08-phase4-priority-stack-v4.md`](docs/research/2026-05-08-phase4-priority-stack-v4.md). |
