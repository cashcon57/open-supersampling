...
v6 is the architecture intended to ship. Today: HAT + empty-canvas identity fusion + pixel head; v6 trainer scaffold runs and produces checkpoints on synthetic input. Stage 2 building blocks (Gaussian spawner, motion-vector + GS-STVSR covariance canvas warp, active-mask-aware rasterizer wrapper) committed in `8c84727` with 31 unit tests passing — none wired into `V6Model.forward()` yet. Pending wire-up: canvas in the HR critical path (replaces pixel head), trajectory training loop with canvas continuity + temporal-consistency loss, OSS-FX integration. The full diagram below is the target architecture.
...
S6 is v6: the covariance-resampled Gaussian-temporal architecture summarized above. Modules, orchestrator, training loop scaffold, and Stage 2 building blocks (spawner, canvas warp, rasterizer wrapper) have landed. The Stage 2 wire-up that puts canvas in the HR critical path, the trajectory training loop with canvas continuity, and OSS-FX integration are still pending. Three tiers (Pico, Standard, Heavy), custom per-vendor kernels, and DLL-shim integration remain target work.
