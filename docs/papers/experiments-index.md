...
| 2026-05-02 | [srcnn-beats-v05-and-gsasr](../superpowers/experiments/2026-05-02-srcnn-beats-v05-and-gsasr.md) | **SR-CNN (clean CNN, no splats) beats V0.5 and GSASR.** Train +2.71 dB; held-out CitySample +2.40, StylizedRendering +1.87, ArchVizInterior +4.05. **64/64 samples beat bicubic.** SR-CNN replaces V0.5 as v0 ship. The day's binding insight: training distribution dominates architecture — any architecture trained on engine-aliased LR beats pretrained models from clean-LR distributions. Splats are dead for SR; the Gaussian track survives in OSS Ray-Retracing (denoising). |
| 2026-05-03 | [onnx-export-and-bench](../superpowers/experiments/2026-05-03-onnx-export-and-bench.md) | **ONNX FP16 shipped as inference path.** 1.4-1.6× speedup vs PyTorch FP32 at SD/720p/900p. TRT not installed at time of this memo — ORT CUDA EP only. |
| 2026-05-03 | [trt-int8-quantization](../superpowers/experiments/2026-05-03-trt-int8-quantization.md) | **TRT FP16 is the primary path; INT8 does NOT win.** TRT FP16: 18.6/15.6/28.0/37.6 ms (SD/720p/900p/1080p). TRT INT8: 21.8/18.6/15.6/50.0 ms — slower at 3 of 4 resolutions. Quality gate PASSED (INT8: +0.46 dB, −0.010 LPIPS vs FP32 on CitySample). Both beat spec targets except INT8 at 1080p (50ms vs 45ms target). Ship TRT FP16. |
| 2026-05-06 | [v5-pixel-temporal-final-held-out-eval](../superpowers/experiments/2026-05-06-v5-pixel-temporal-final-held-out-eval.md) | **v5-pixel-temporal cleared every spec exit gate on TartanAir oldtown held-out (64 pairs).** PSNR 25.703 dB (+1.794 vs bicubic 23.909). LPIPS-VGG 0.1666 (−43.4% vs bicubic 0.2945). 64/64 beats bicubic on PSNR AND LPIPS (spec target ≥95%). Temporal-stability ratio B/A = 0.337 (spec target ≤0.5, 1.5× margin). Initial eval reported a buggy 13.858 dB / 0.566 LPIPS because the held-out script did not honor the conditional `zero_gbuffer_into_backbone` flag; fix in commit `d8ef408`. v5 LPIPS at 0.1666 is in the same band as DLSS 4's published ~0.17 on AAA games. Caveat: TartanAir is friendlier than AAA content, so this is NOT a "v5 beats DLSS 4" claim. It IS the first OSS model demonstrably in DLSS-class quality territory on the held-out content class. |
...
| 2026-05-03 | onnx-export-and-bench, trt-int8-quantization                                                                                                                               |
