...
**Last updated:** 2026-05-04 23:04 CDT
...
- `b4f4023` v5-pixel(sr): scaffold stateless model wrapper for ONNX export + design memo
...
- **C23 — done by Codex at 21:58 CDT.** Capture modes client patch after Round 8: `OssCaptureMode` added with `trickle`, `lite`, `regular`, and `INSANE`; `oss_capture_apply_mode_preset()` fills documented burst/channel/static-trigger/INSANE fields. Default config is `lite`. Trickle keeps LR+HR+depth+motion+normals for static singles, drops only HR on opportunistic pair `t+1`, and does not emit burst fields for static singles. Lite keeps C22 bursts and adds static singles only. Regular adds albedo+roughness with denser bursts. INSANE adds metallic+emissive plus supersample/DLAA/multi-DLSS/scene-cut flags. `exr_writer.cpp` now selects channels from capture flags. Fixture JSON now includes `capture_mode`, with trickle static singles omitting all `burst_*` fields. Verification: focused capture tests → 14 passed; full `tests/capture -q -rs` → 21 passed, 23 skipped due missing `moto`; C++ syntax-only capture compile passed; `git diff --check` passed. Server files were not touched by Codex.
- **C24 — done by Codex at 23:04 CDT.** `scripts/build_capture_installer.py` now accepts `--mode {trickle,lite,regular,INSANE}` defaulting to `lite`, validates/bakes `capture_mode` into `config.json`, and includes the INSANE-only 256-frame supersample-GT consent disclosure in config/manifest. Commit: `96b380a`.
- **C25 — done by Codex at 23:04 CDT.** `oss/capture/uploader.py` now treats HTTP 429 as retryable, honors `Retry-After` when present, falls back to 4x backoff when absent, and still deletes after `max_attempts` exhaustion. Commit: `f67ba8c`.

R9 verification: `venv-py312/bin/python -m pytest tests/capture/test_r2_layout.py -q -rs` → 19 passed; `venv-py312/bin/python -m pytest tests/capture/test_uploader.py -q` → 9 passed; full `venv-py312/bin/python -m pytest tests/capture -q -rs` → 59 passed, 1 skipped (`pyarrow` missing for capture-index test); `git diff --check` passed.
