...
- Sprint 5 (persistent canvas) revives in this context — temporal accumulation of per-frame Gaussians is exactly what RR wants.
- Sprint 6 (frame extrapolation) is similarly relevant — extrapolation needs a usable per-frame denoised buffer.

## What's NOT blocked

OSS-SR ships independently as a CNN-based pipeline. RR work happens in parallel (or sequentially after SR ships) without delaying the SR product.
...
3. Can the same network from Sprint 4 (after the dead-init fixes from `2026-05-02-output-head-dead-init.md`) be repurposed for RR by changing the input channels and loss, or do we want a fresh architecture? Almost certainly the former — the Sprint 4 module is tier-scalable and well-tested.
